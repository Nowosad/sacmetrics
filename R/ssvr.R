...
#' ssvr_value = ssvr(vgm_model)
#' ssvr_value
ssvr = function(model) {
  if (!is.data.frame(model)) {
    stop("'model' must be a vgm-like data.frame with columns 'model' and 'psill'")
  }
  ...
  nugget_rows = which(tolower(as.character(model$model)) %in% c("nug", "nugget"))
  nugget_psill = if (length(nugget_rows) > 0) sum(model$psill[nugget_rows]) else 0
