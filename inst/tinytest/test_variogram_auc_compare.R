# Tests for variogram_auc_compare() function

# Test similarity of identical models
vgm_model = data.frame(model = c("Nug", "Sph"), psill = c(0.5, 1.5), range = c(0, 100))
sim_identical = variogram_auc_compare(vgm_model, vgm_model, maxdist = 100)
...
sim_value = variogram_auc_compare(vgm_model1, vgm_model2, maxdist = 150)
...
sim_different = variogram_auc_compare(vgm_small, vgm_large, maxdist = 200)
expect_true(sim_different < 1)

# Test warning when n is even
expect_warning(
  variogram_auc_compare(vgm_model1, vgm_model2, maxdist = 100, n = 100),
  pattern = "odd"
)
...
sim_inferred = variogram_auc_compare(vgm_a, vgm_b)
